...
public class RCC_CarControllerV3 : RCC_Core {
	...
	public bool overrideWheelAll = false;		//	Overriding individual wheel settings such as steer, power, brake, handbrake.
	...
	public float antiRollVertical = 0f;													// Anti Roll Vertical Force For Preventing Flip Overs And Stability. I know it doesn't exist, but it can improve gameplay if you have high COM vihecles like monster trucks.
	#endregion
	...
	[Range(.05f, 1f)] public float steeringSensitivityFactor = .5f;		// Steering sensitivity multiplier.
	...
	public bool useAutomaticClutch { get { return RCCSettings.useAutomaticClutch; } }
	#endregion
	...
	public bool useDeformation = true;		// Use deformation on collisions.
	...
	void Awake (){
		...
		allWheelColliders = GetComponentsInChildren<RCC_WheelCollider>(true);
	...
	private void Inputs() {
		...
		if (RCCSettings.steeringWheelEnabled) {
        ...
        if (((1 << collision.gameObject.layer) & damage.damageFilter) != 0) {
            ...
            if (useDeformation) {
                ...
                if (!damage.initialized) {
                    ...
                    damage.Initialize(this, allMeshFilters);
            ...
            if (useCollisionAudio) {
                ...
                if (crashClips.Length > 0) {
                    ...
                    if (collision.contacts[0].thisCollider.gameObject.transform != transform.parent) {
                        ...
                        crashSound = NewAudioSource(RCCSettings.audioMixer, gameObject, "Crash Sound AudioSource", 5f, 20f, RCCSettings.maxCrashSoundVolume, crashClips[UnityEngine.Random.Range(0, crashClips.Length)], false, true, true);
            ...
            if (useCollisionParticles) {
                ...
                for (int i = 0; i < contactSparkeList.Count; i++) {
                    ...
                    if (!contactSparkeList[i].isPlaying) {
                        ...
                        contactSparkeList[i].transform.position = collision.contacts[0].point;
